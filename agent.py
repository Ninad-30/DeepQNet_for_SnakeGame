...
def train():
    ...
    while True:
        ...
        if done:
            ...
            if agent.n_games % 250 == 0:
                plt.clf()
                plt.title(f'Trained for {agent.n_games} games')
                ...
                plt.show()
                plt.savefig(f'plots/trained_{agent.n_games}games_plot.png')
                print(f"Successfully saved training plot after {agent.n_games} games!")
